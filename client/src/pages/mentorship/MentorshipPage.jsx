...
const MentorshipPage = () => {
  ...
  useEffect(() => {
    const fetchMentors = async () => {
      try {
        ...
        const uniqueCategories = [...new Set(mentorData.map(m => m.category))];
        const uniqueExpertise = [...new Set(mentorData.map(m => m.expertiseLevel))];
  ...
  useEffect(() => {
    ...
    if (selectedExpertise !== 'all') {
      filtered = filtered.filter(mentor => mentor.expertiseLevel === selectedExpertise);
    }
  ...
  const getCategoryIcon = (category) => {
    switch (category) {
      ...
      case 'Mobile Development': return <Phone className="w-5 h-5" />;
      case 'Data Science': return <BookOpen className="w-5 h-5" />;
      case 'UI/UX Design': return <Award className="w-5 h-5" />;
      case 'Digital Marketing': return <Globe className="w-5 h-5" />;
      case 'Project Management': return <Briefcase className="w-5 h-5" />;
      case 'Career Development': return <User className="w-5 h-5" />;
  ...
  const getExpertiseColor = (level) => {
    switch (level) {
      ...
      case 'Thought Leader': return 'bg-red-100 text-red-800';
  ...
  const getAvailabilityColor = (status) => {
    switch (status) {
      ...
      case 'Do Not Disturb': return 'text-red-600 bg-red-100';
  ...
  const formatPrice = (mentor) => {
    ...
    if (mentor.pricing.sessionRate > 0) return `KES ${mentor.pricing.sessionRate.toLocaleString()}/session`;
    if (mentor.pricing.hourlyRate > 0) return `KES ${mentor.pricing.hourlyRate.toLocaleString()}/hour`;
    if (mentor.pricing.monthlyRate > 0) return `KES ${mentor.pricing.monthlyRate.toLocaleString()}/month`;
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container-custom">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-ajira-primary mb-4">Find Your Perfect Mentor</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-8">
            Connect with expert mentors in real-time for instant help, career guidance, and skill development. 
            Get matched with the right mentor in minutes, just like requesting a ride!
          </p>
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className={`font-semibold ${
                  ...
                  {requestStatus.type === 'success' ? '🚀 Request Submitted!' : '❌ Request Failed'}
                </h3>
                <p className={`mt-2 ${
                ...
                {requestStatus.data && (
                  <div className="mt-4 p-4 bg-white rounded-lg">
                    <p className="text-sm text-gray-600">
                      <strong>Request ID:</strong> {requestStatus.data.requestId}<br/>
                      <strong>Mentors Notified:</strong> {requestStatus.data.mentorsNotified}<br/>
                      <strong>Estimated Response:</strong> {requestStatus.data.estimatedResponseTime}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900">Request a Mentor</h2>
                <button
              ...
              <form onSubmit={submitMentorRequest} className="space-y-6">
                ...
                <div className="flex items-center space-x-4">
                  <label className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-sm font-medium text-gray-700">I have a budget</span>
                  </label>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
            <span>{filteredMentors.length} mentor{filteredMentors.length !== 1 ? 's' : ''} available</span>
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{mentor.mentor.title}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getAvailabilityColor(mentor.availability.status)}`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium text-gray-900">{mentor.availability.responseTime}</span>
                  </div>

                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-500">Sessions:</span>
                    <span className="text-sm font-medium text-gray-900">{mentor.statistics.totalSessions}</span>
                  </div>
                  ...
                  {mentor.ratings.totalRatings > 0 && (
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm font-medium text-gray-900">
                          {mentor.ratings.overall} ({mentor.ratings.totalRatings})
                        </span>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  ...
                  <div className="flex space-x-2">
                    <button 
                      className="px-3 py-2 text-sm border border-ajira-primary text-ajira-primary rounded-lg hover:bg-ajira-primary/5 transition-colors"
                    >
                    ...
                    >
                      {mentor.availability.status === 'Available' ? 'Request Help' : mentor.availability.status}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {filteredMentors.length > 0 && (
          <div className="bg-gradient-to-r from-ajira-primary to-ajira-secondary rounded-lg p-8 text-center">
            ...
            <p className="text-white/90 mb-6 max-w-2xl mx-auto">
              Get matched with an available mentor instantly. Our Uber-like system connects you 
              with the perfect mentor based on your needs, location, and budget in minutes.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button 
                onClick={() => setShowRequestForm(true)}
