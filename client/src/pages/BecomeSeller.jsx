...
const BecomeSeller = () => {
  ...
  const [conversation, setConversation] = useState([
    {
      role: 'assistant',
      content: "Hello! I'm Kinap Ajira Assistant, your Seller Profile Builder. I'll help you create a professional seller profile that stands out in the marketplace. Let's start with your basic information. What's your full name?"
    }
  ]);
  ...
  const handleAnswer = async (answer) => {
    ...
    setTimeout(() => {
      ...
      if (question.id === 'fullName') {
        aiResponse = "Great! Now let's get your professional title. What professional title best describes you?";
      } else if (question.id === 'title') {
        aiResponse = "Excellent! Now let's identify your key skills. What are your top 5-7 skills?";
        ...
        aiResponse = "Impressive! What services do you want to offer on our marketplace?";
        ...
        aiResponse = "Great! Can you share links to 3-5 of your best projects or portfolio pieces?";
      } else if (question.id === 'portfolio') {
        aiResponse = "Excellent! What are your biggest professional achievements?";
      } else if (question.id === 'achievements') {
        aiResponse = "Perfect! Let me analyze your profile and create your professional assessment.";
      }

      // Add AI response to conversation
      setConversation(prev => [...prev, { role: 'assistant', content: aiResponse, suggestions }]);
  ...
  const generateFinalAnalysis = async () => {
    ...
    setTimeout(() => {
      ...
      const totalPortfolioItems = portfolioLinks.length + portfolioImages.length;
      const portfolioScore = totalPortfolioItems >= 5 ? 3 : totalPortfolioItems >= 3 ? 2 : totalPortfolioItems >= 1 ? 1 : 0;
      const skillsScore = skills.length >= 5 ? 2 : skills.length >= 3 ? 1 : 0;
      const experienceScore = experience.toLowerCase().includes('year') && parseInt(experience.match(/\d+/)?.[0] || 0) >= 2 ? 2 : 1;
      const pricingScore = pricing && parseInt(pricing) >= 15 ? 1 : 0;
      
      const totalScore = portfolioScore + skillsScore + experienceScore + pricingScore;
      const rating = Math.min(10, Math.max(5, Math.floor(totalScore * 1.5) + 5));
      
      // Generate strengths based on actual data
      const strengths = [];
      if (totalPortfolioItems >= 5) strengths.push('Excellent portfolio with multiple examples and images');
      else if (totalPortfolioItems >= 3) strengths.push('Good portfolio with multiple examples');
      if (portfolioImages.length > 0) strengths.push('Visual portfolio with project images');
      if (skills.length >= 5) strengths.push('Strong skill set with diverse capabilities');
      if (experience.toLowerCase().includes('year') && parseInt(experience.match(/\d+/)?.[0] || 0) >= 3) strengths.push('Solid professional experience');
      if (pricing && parseInt(pricing) >= 20) strengths.push('Competitive pricing strategy');
      if (strengths.length === 0) strengths.push('Good foundation for growth');
      
      // Generate improvements based on gaps
      const improvements = [];
      if (totalPortfolioItems < 3) improvements.push('Add more portfolio examples (aim for 3-5 total items)');
      if (portfolioImages.length === 0) improvements.push('Add project images to showcase your work visually');
      if (skills.length < 5) improvements.push('Expand your skill set to increase marketability');
      if (!experience.toLowerCase().includes('year')) improvements.push('Highlight your years of experience');
      if (!pricing || parseInt(pricing) < 15) improvements.push('Consider competitive pricing for your market');
      if (improvements.length === 0) improvements.push('Continue building client testimonials');
      ...
      const analysis = {
        ...
        portfolioAnalysis: totalPortfolioItems > 0 ? 
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      const applicationData = {
        ...
        portfolioLinks: Array.isArray(formData.portfolio) ? formData.portfolio : [],
        portfolioImages: Array.isArray(formData.portfolioImages) ? formData.portfolioImages : [],
        ...
        serviceDescription: Array.isArray(formData.services) ? formData.services.join(', ') : '',
        valueProposition: `Professional ${formData.title || 'Seller'} with ${formData.experience || 'experience'}`,
        sampleWork: Array.isArray(formData.portfolio) ? formData.portfolio.join(', ') : '',
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {formData[question.id]?.map((tag, index) => (
                <span key={index} className="bg-ajira-primary text-white px-3 py-1 rounded-full text-sm">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onKeyPress={(e) => {
                  if (e.key === 'Enter' && e.target.value.trim()) {
                    const newLinks = [...(formData[question.id] || []), e.target.value.trim()];
                    setFormData(prev => ({ ...prev, [question.id]: newLinks }));
                    e.target.value = '';
                  }
                }}
              />
            ...
            <div className="text-sm text-gray-600">
              💡 Tip: You can add links to your Behance, Dribbble, GitHub, personal website, or upload project images directly
            </div>

            {/* Display Portfolio Links */}
            {formData[question.id]?.length > 0 && (
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">Added Links:</label>
                <div className="flex flex-wrap gap-2">
                  {formData[question.id]?.map((link, index) => (
                    <div key={index} className="flex items-center bg-ajira-primary text-white px-3 py-1 rounded-full text-sm">
                      <span className="truncate max-w-32">{link}</span>
                      <button
                        onClick={() => {
                          const newLinks = formData[question.id].filter((_, i) => i !== index);
                          setFormData(prev => ({ ...prev, [question.id]: newLinks }));
                        }}
  ...
  const renderProfileBuilder = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-ajira-primary mb-2">Building Your Profile</h2>
        <p className="text-gray-600">Let's create a professional seller profile together</p>
      </div>
      
      {/* AI Conversation Display */}
      <div className="bg-gray-50 rounded-lg p-4 max-h-64 overflow-y-auto">
        <h3 className="font-semibold text-ajira-primary mb-3">AI Assistant Conversation</h3>
        <div className="space-y-3">
              ...
              <div className={`max-w-xs lg:max-w-md px-3 py-2 rounded-lg ${
                ...
                {msg.suggestions && msg.suggestions.length > 0 && (
                  <div className="mt-3 space-y-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <p className="text-sm text-blue-800 font-medium">💡 AI Suggestions - Click to add:</p>
                    <div className="flex flex-wrap gap-2">
      ...
      {conversation.length > 0 && conversation[conversation.length - 1]?.suggestions && (
        <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <h4 className="font-medium text-yellow-800 mb-2">🎯 AI Suggestions Available!</h4>
          <p className="text-sm text-yellow-700 mb-3">
            Check the conversation above to see AI suggestions for your current question.
          </p>
          <button
            onClick={() => {
  ...
  const renderAnalysis = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-ajira-primary mb-2">AI Analysis Complete</h2>
      ...
      {aiAnalysis && (
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="text-3xl font-bold text-ajira-primary">{aiAnalysis.rating}/10</div>
            <p className="text-sm text-gray-600 mt-2">Professional Profile Score</p>
            <div className="mt-3 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">{aiAnalysis.portfolioAnalysis}</p>
            </div>
          </div>
  ...
  const renderSuccess = () => (
    <div className="text-center space-y-6">
      ...
      <h2 className="text-2xl font-bold text-ajira-primary">Application Submitted!</h2>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-gray-600">
                  {step === 1 ? 'Getting Started' : 
                   step === 2 ? `Question ${currentQuestion + 1} of ${questions.length}` :
                   step === 3 ? 'AI Analysis' : 'Complete'}
                </span>
                <span className="text-sm font-medium text-gray-600">
                  {step === 1 ? '25%' : 
                   step === 2 ? `${Math.round(((currentQuestion + 1) / questions.length) * 50) + 25}%` :
                   step === 3 ? '75%' : '100%'}
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  style={{ 
                    width: step === 1 ? '25%' : 
                           step === 2 ? `${((currentQuestion + 1) / questions.length) * 50 + 25}%` :
                           step === 3 ? '75%' : '100%'
                  }}
                ></div>
              </div>
              ...
              <>
                {step === 1 && (
                  <div className="text-center space-y-6">
                    ...
                    <h1 className="text-3xl font-bold text-ajira-primary">Become a Seller</h1>
