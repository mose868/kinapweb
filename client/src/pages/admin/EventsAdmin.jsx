...
const EventsAdmin = () => {
  ...
  const { data: attendeesData } = useQuery(
    ...
    async () => {
      ...
      const response = await axios.get(`${BASEURL}/events/admin/${selectedEventId}/attendees`);
      return response.data;
    },
  ...
  const saveEventMutation = useMutation(
    async (data) => {
      if (editingEvent) {
        const response = await axios.put(`${BASEURL}/events/admin/${editingEvent._id}`, {
          ...data,
  ...
  const togglePublishMutation = useMutation(
    async (id) => {
      const response = await axios.patch(`${BASEURL}/events/admin/${id}/publish`);
      return response.data;
    },
    {
      onSuccess: () => {
  ...
  const toggleFeaturedMutation = useMutation(
    async (id) => {
      const response = await axios.patch(`${BASEURL}/events/admin/${id}/feature`);
      return response.data;
    },
    {
      onSuccess: () => {
  ...
  const updateStatusMutation = useMutation(
    async ({ id, status }) => {
      const response = await axios.patch(`${BASEURL}/events/admin/${id}/status`, { status });
      return response.data;
    },
    {
      onSuccess: () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Registration Open': return 'text-blue-600 bg-blue-100';
      case 'Registration Closed': return 'text-yellow-600 bg-yellow-100';
  ...
  const getTypeColor = (type) => {
    switch (type) {
      case 'In-Person': return 'text-blue-600 bg-blue-100';
      case 'Virtual': return 'text-green-600 bg-green-100';
  ...
  const getCategoryIcon = (category) => {
    switch (category) {
      ...
      case 'Networking': return <UserCheck className="w-4 h-4" />;
      case 'Hackathon': return <Activity className="w-4 h-4" />;
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg p-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-ajira-primary">Events Management</h1>
          <button
            ...
            <div className="bg-ajira-primary/10 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-ajira-primary">{eventStats.eventStats?.totalEvents || 0}</p>
                </div>
              </div>
            </div>
            <div className="bg-green-100 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{eventStats.eventStats?.publishedEvents || 0}</p>
                </div>
              </div>
            </div>
            <div className="bg-blue-100 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{eventStats.eventStats?.upcomingEvents || 0}</p>
                </div>
              </div>
            </div>
            <div className="bg-purple-100 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600">{eventStats.eventStats?.totalRegistrations || 0}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <div key={event._id} className="bg-white border border-gray-200 rounded-lg p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-3">
                        ...
                        <span className="flex items-center space-x-1">
                          ...
                          <span>{event.pricing.isFree ? 'Free' : `KES ${event.pricing.regularPrice}`}</span>
                        </span>
                      </div>
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      ...
                      <select
                        value={event.status}
                        onChange={(e) => updateStatusMutation.mutate({ id: event._id, status: e.target.value })}
          ...
          <div className="space-y-6">
                ...
                >
                  ...
                  {filteredEvents.map(event => (
                    <option key={event._id} value={event._id}>
                      {event.title} ({new Date(event.schedule.startDate).toLocaleDateString()})
                    </option>
                  ))}
                </select>
                {selectedEventId && (
            ...
            {attendeesData && (
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200">
                  <h3 className="text-lg font-semibold">{attendeesData.eventTitle}</h3>
                        ...
                        <tr key={index}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                              ...
                              attendee.status === 'Attended' ? 'bg-purple-100 text-purple-800' :
                              attendee.status === 'No Show' ? 'bg-red-100 text-red-800' :
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                              attendee.paymentStatus === 'Paid' ? 'bg-green-100 text-green-800' :
                              attendee.paymentStatus === 'Free' ? 'bg-blue-100 text-blue-800' :
            ...
            {!selectedEventId && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">Choose an event to view its attendees</p>
              </div>
            )}
          </div>
        )}
                  ...
                  {eventStats.eventStats.popularEvents.map((event, index) => (
                    <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-right">
                        <p className="font-semibold text-gray-900">{event.analytics.views} views</p>
                        <p className="text-sm text-gray-600">{event.registration.registered} registered</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'events' && filteredEvents.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              {searchQuery ? 'No events match your search criteria.' : 'Start building your event calendar by adding the first event.'}
            </p>
            {!searchQuery && (
              <button
