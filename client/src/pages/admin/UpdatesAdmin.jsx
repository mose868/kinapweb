...
const UpdatesAdmin = () => {
  ...
  const { data: updateStats } = useQuery('updateStats', async () => {
    ...
    async (data) => {
      if (editingUpdate) {
        const response = await axios.put(
          `${BASEURL}/updates/${editingUpdate._id}`,
          {
            ...data,
    ...
    const processedData = {
      ...
      eventDetails: {
        ...
        eventDate: formData.eventDetails.eventDate
          ? new Date(formData.eventDetails.eventDate)
          : null,
        capacity: formData.eventDetails.capacity
          ? parseInt(formData.eventDetails.capacity)
          : null,
      },
    };
              ...
              <div className='flex items-start justify-between'>
                <div className='flex-1'>
                  ...
                  <div className='flex flex-wrap items-center gap-4 text-sm text-gray-500'>
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(update.priority)}`}
                    >
                      {update.priority}
                    </span>
                ...
                <div className='flex items-center space-x-2 ml-4'>
                  ...
                  >
                    <Star
                      className={`w-4 h-4 ${update.featured ? 'fill-current' : ''}`}
                    />
                  </button>
