...
const EventsPage = () => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const metaResponse = await axios.get(
          `${BASEURL}/events/meta/categories`
        );
  ...
  useEffect(() => {
    ...
    if (selectedCity !== 'all') {
      filtered = filtered.filter(
        (event) => event.location.city === selectedCity
      );
    }
  ...
  const getAvailabilityStatus = (event) => {
    ...
    const regDeadline = event.registration.registrationDeadline
    ...
    if (!event.registration.isRegistrationOpen)
      return { text: 'Registration Closed', color: 'text-red-600 bg-red-100' };
    if (regDeadline && now > regDeadline)
      return {
        text: 'Registration Deadline Passed',
  ...
  const submitRegistration = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${BASEURL}/events/${selectedEvent._id}/register`,
        registrationData
      );
      ...
      const updatedEvents = events.map((event) =>
        event._id === selectedEvent._id
          ? {
              ...
              registration: {
                ...event.registration,
                registered: event.registration.registered + 1,
              },
            }
          : event
      );
    ...
    <div className='min-h-screen bg-gray-50 py-8 sm:py-12 w-full overflow-x-hidden'>
      <div className='container-custom px-2 sm:px-4 w-full'>
            ...
            <div className='bg-white rounded-lg p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto'>
              ...
              <div className='mb-6 p-4 bg-gray-50 rounded-lg'>
                ...
                <p className='text-sm text-gray-600 mt-1'>
                  {formatDate(selectedEvent.schedule.startDate)} •{' '}
                  {formatTime(selectedEvent.schedule.startTime)} -{' '}
        ...
        <div
          className={
            viewMode === 'grid'
              ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 mb-12 w-full'
              : 'space-y-6 mb-12 w-full'
          }
        >
          {filteredEvents.map((event) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`bg-gradient-to-r from-ajira-primary to-ajira-secondary ${
                ...
                <div className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                  ...
                  <div className='flex items-start justify-between mb-4'>
                    <div className='flex-1'>
                      ...
                      <div className='flex items-center space-x-2 mb-2'>
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${availability.color}`}
                        >
                          {availability.text}
                        </span>
                      </div>
                  ...
                  <div className='space-y-3 mb-4'>
                    <div className='flex items-center text-sm text-gray-500'>
                      ...
                      <span>
                        {formatDate(event.schedule.startDate)} • {timeUntil}
                      </span>
                    </div>
                    ...
                    <div className='flex items-center text-sm text-gray-500'>
                      ...
                      <span>
                        {event.pricing.isFree
                          ? 'Free'
                          : `KES ${event.pricing.regularPrice.toLocaleString()}`}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className='flex space-x-3'>
                    <button className='flex-1 px-4 py-2 text-sm border border-ajira-primary text-ajira-primary rounded-lg hover:bg-ajira-primary/5 transition-colors'>
                    ...
                    <button
                      ...
                      disabled={
                        availability.text === 'Past Event' ||
                        availability.text === 'Registration Closed'
                      }
                    ...
                    >
                      ...
                      {availability.text === 'Waitlist Available'
                        ? 'Join Waitlist'
                        : 'Register'}
                    </button>
                  </div>
        ...
        {filteredEvents.length > 0 && (
          <div className='bg-gradient-to-r from-ajira-primary to-ajira-secondary rounded-lg p-4 sm:p-8 text-center w-full'>
            ...
            <p className='text-white/90 mb-6 max-w-2xl mx-auto text-sm sm:text-base'>
              Stay updated with the latest events, workshops, and networking
              opportunities. Join our community and never miss an event that
