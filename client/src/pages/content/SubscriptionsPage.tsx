  ...
  return (
    <div className="min-h-screen bg-white px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="text-gray-500 text-lg">You have not subscribed to any channels yet.</div>
      ) : (
        <>
          <div className="mb-6 flex flex-wrap gap-4">
            {subscriptions.map(channel => (
              <div key={channel} className="flex items-center gap-2 bg-gray-100 px-4 py-2 rounded-full">
                <img src={onlineWorkVideos.find(v => v.channel.name === channel)?.channel.avatar || DEFAULT_AVATAR} alt={channel} className="w-6 h-6 rounded-full" />
                <span className="text-sm font-medium text-gray-800">{channel}</span>
              </div>
            ))}
          </div>
          {subscribedVideos.length === 0 ? (
            <div className="text-gray-400">No videos from your subscriptions yet.</div>
          ) : (
