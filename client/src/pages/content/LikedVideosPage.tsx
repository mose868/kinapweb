...
import { getLikedVideos, removeLikedVideo } from '../../api/userVideos';
...
const LikedVideosPage = () => {
  ...
  async function handleRemove(video) {
    await removeLikedVideo(video);
    fetchLikedVideos();
  }

  return (
    <div className="min-h-screen bg-white px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-gray-900">Liked Videos</h1>
      {loading ? (
        <div className="text-gray-500 text-lg">Loading...</div>
      ) : likedVideos.length === 0 ? (
        <div className="text-gray-500 text-lg">No liked videos yet.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {likedVideos.map(video => (
            <div key={video.id} className="bg-white rounded-xl shadow hover:shadow-lg transition-shadow cursor-pointer overflow-hidden group relative">
              ...
              <button onClick={() => handleRemove(video)} className="absolute top-2 right-2 bg-white text-gray-700 rounded-full p-1 shadow hover:bg-red-100 hover:text-red-600 transition-colors">
                Remove
              </button>
            </div>
          ))}
