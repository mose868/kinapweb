...
const demoChats = [
  ...
  },
  {
    ...
    unread: 510,
    avatar:
      'https://ui-avatars.com/api/?name=NASDQ&background=FF6B35&color=fff',
  },
];
...
const CommunityChat: React.FC = () => {
  ...
  return (
    <div
      className='w-full min-h-screen flex flex-col md:flex-row bg-white overflow-x-hidden'
    ...
    >
        ...
        <div className='flex-1 overflow-y-auto'>
          {demoChats.map((chat) => (
            ...
            >
              ...
              {chat.unread > 0 && (
                <span className='bg-blue-500 text-white text-xs px-2 py-1 rounded-full ml-2 min-w-[28px] text-center'>
      ...
      {/* Main Chat Area */}
      <div
        className="flex-1 flex flex-col h-full bg-[url('https://www.transparenttextures.com/patterns/symphony.png')] bg-repeat relative w-full"
      ...
      >
        ...
        {/* Messages */}
        <div
          className='flex-1 overflow-y-auto px-2 sm:px-0 py-4 sm:py-6 w-full'
          style={{ background: 'none', minHeight: '200px' }}
        >
          {/* Unread separator */}
          <div className='flex justify-center my-2'>
            <span className='bg-green-200 px-3 py-1 rounded text-xs font-bold shadow'>
              Unread Messages
            </span>
          </div>
          {/* Date separator */}
          <div className='flex justify-center my-2'>
            <span className='bg-white px-3 py-1 rounded text-xs text-gray-500 shadow'>
              July 7
            </span>
          </div>
          {/* Messages */}
          ...
          {messages.map((msg, i) => (
            <div
              key={i}
              className={`mb-2 flex ${msg.self ? 'justify-end' : 'justify-start'} px-2 sm:px-6`}
            >
              <div
                className={`max-w-[90vw] sm:max-w-xs md:max-w-md px-4 py-2 rounded-2xl shadow ${msg.self ? 'bg-blue-500 text-white rounded-br-none' : 'bg-white text-gray-900 rounded-bl-none'}`}
              >
        ...
        <div className='h-20 px-2 sm:px-6 py-3 bg-white flex items-center gap-2 border-t sticky bottom-0 z-10 w-full'>
          <button className='text-xl text-gray-500 hover:text-blue-500'>
            <FaSmile />
          </button>
          <button className='text-xl text-gray-500 hover:text-blue-500'>
            <FaPaperclip />
          ...
          />
          <button
            className='bg-blue-500 text-white px-3 sm:px-4 py-2 rounded font-semibold hover:bg-blue-600 text-sm sm:text-base'
