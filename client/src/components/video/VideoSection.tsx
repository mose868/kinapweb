...
const YOUTUBE_API_KEY =
  import.meta.env.VITE_YOUTUBE_API_KEY || 'your_youtube_api_key'; // <-- Replace with your real API key
  ...
  const fetchVideos = async (query: string) => {
    ...
    try {
      ...
      const searchRes = await fetch(
        `${YOUTUBE_SEARCH_URL}?part=snippet&type=video&maxResults=12&q=${encodeURIComponent(query)}&key=${YOUTUBE_API_KEY}`
      );
      ...
      const detailsRes = await fetch(
        `${YOUTUBE_VIDEO_URL}?part=snippet,contentDetails,statistics,liveStreamingDetails&id=${videoIds}&key=${YOUTUBE_API_KEY}`
      );
    ...
    <div className='bg-white py-4 w-full min-h-screen'>
      ...
      <div className='sticky top-0 z-40 bg-white w-full'>
        <form
          ...
          className='flex flex-col sm:flex-row items-center gap-2 mb-2 w-full px-2 pt-2'
        >
          <input
            type='text'
            value={searchTerm}
          ...
          <button
            type='submit'
            className='bg-[#15325C] text-white px-4 py-2 rounded-lg flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0 text-sm'
          >
            <Search size={18} />
            Search
          </button>
      ...
      {loading && (
        <div className='text-center py-8 text-ajira-accent font-bold'>
          Loading videos...
        </div>
      )}
